 ...
 */
class TeamImpl extends Team implements EventPublisher{
    ...
    boolean publishEvent(Event event) {
        ...
        ((EventPublisher)player).publishEvent(event)
        ((EventPublisher)mainTower).publishEvent(event)
        ((EventPublisher)middleTower).publishEvent(event)
        ((EventPublisher)outerTower).publishEvent(event)
    }

    @Override
    boolean removeListener(Class clazz) {
        publisher.removeListener(clazz)
    }
}
